...
model = model.cuda()
model.eval()
# 假设 agent_attention_layer 是你的 AgentAttention 层的正确路径


agent_attention_layer = model.module.agent
# agent_attention_layer.register_forward_hook(get_activation('agent_attention'))
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm_gen, 1):
                ...
                loss = args.alpha * F.cross_entropy(logits, label) + (1 - args.alpha) * F.cross_entropy(logits_trans, label)
                # loss = F.cross_entropy(logits, label)
                # y_oh = utils.one_hot(label.cpu(), 5)
                # loss = args.alpha * F.cross_entropy(logits, label)

                # y_oh = Variable(y_oh.cuda())
                # loss = F.mse_loss(logits, y_oh)
                acc = count_acc(logits, label)

                # acc2 = count_acc(logits_trans, label)
                # acc = (acc1 + acc2)/2
                vl.add(loss.item())
                va.add(acc)
