...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, mem):  # torch.Size([1, nq, c]) torch.Size([1, ns, c])
        ...
        score1 =  self.fc( query.squeeze(0)).softmax(dim=-1)
...
class RelationNetwork(nn.Module):
    ...
    def __init__(self,input_size,hidden_size):
        ...
        self.layer1 = nn.Sequential(
                        ...
                        nn.BatchNorm2d(640, momentum=1, affine=True),
                        nn.ReLU(),
                        nn.MaxPool2d(2))
        ...
        #     nn.ReLU(),
        #     nn.MaxPool2d(2))
        self.fc1 = nn.Linear(640,hidden_size)
...
class RelationModule(nn.Module):
    ...
    def forward(self,x):
        out = self.layer1(x)
        out = self.layer2(out)
        out = out.view(out.size(0),-1)#375,640
        out = F.relu(self.fc1(out))#375,8
...
class DeepEMD(nn.Module):
    ...
    def forward(self, input):
        if self.mode == 'meta':
            ...
            # 75,5
            z_proto_ext = z_support_save.repeat(75, 1, 1, 1, 1)
            # 5,75
            z_query_ext = z_query.unsqueeze(0).repeat(5, 1, 1, 1, 1)
            z_query_ext = torch.transpose(z_query_ext, 0, 1)
            extend_final_feat_dim = self.feat_dim.copy()
            extend_final_feat_dim[0] *= 2
            relation_pairs = torch.cat((z_proto_ext, z_query_ext), 2).view(-1, 1280, 5, 5)
            score = self.relation_module(relation_pairs).view(-1, 5)
