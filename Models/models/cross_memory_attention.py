...
class DWConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pw_conv(x)
        return x
class CrossMemoryAtt(nn.Module):
    '''
    Cross Memory Attention Module
    '''
    def __init__(self, dim=512, num_heads=8, agent_num=25, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
        ...
        self.ah_bias = nn.Parameter(torch.zeros(1, num_heads, agent_num, self.window, 1))
        self.aw_bias = nn.Parameter(torch.zeros(1, num_heads, agent_num, 1, self.window))
        self.ha_bias = nn.Parameter(torch.zeros(1, num_heads, self.window, 1, agent_num))
        self.wa_bias = nn.Parameter(torch.zeros(1, num_heads, 1, self.window, agent_num))
